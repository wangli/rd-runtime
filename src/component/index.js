...
class AppComponent {
    ...
    // 返回组件事件（含默认事件）
    getEvents(name) {
        return [...DataOptions.events, ...this.getMyEvents(name)]
    }
    // 添加组件（放入未安装列表）
    add(com) {
        if (!com) return false;
        ...
        if (component && component.name) {
            if (this.iComponents[component.name]) {
                ...
                if (this.app && this.app.vapp) {
                    this.app.vapp.component(component.name, item)
                } else {
    ...
    reload() {
        ...
        items.forEach(component => {
            if (this.app && this.app.vapp) {
                this.iComponents[component.name] = merge({}, baseComponent, component)
